...
router.post('/notify-call', async (req, res) => {
  try {
    const { providerId, channelName, callerName, mode, callType, userId } = req.body;
    ...
    const callData = {
      callId: `call_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
      providerId,
      userId: userId || 'unknown', // Store userId for targeting decline notification
      channelName,
      callerName,
      mode: mode || 'audio',
      callType: callType || 'regular',
...
router.post('/accept-call/:callId', async (req, res) => {
  try {
    ...
    if (io) {
      ...
      console.log(`✅ Call ${callId} accepted - notifications sent`);
    }
...
// CRITICAL FIX: Decline call with proper user notification
router.post('/decline-call/:callId', async (req, res) => {
  try {
    const { callId } = req.params;
    ...
    const channelToSockets = req.app.get('channelToSockets');

    console.log('📢 Preparing to send decline notification for:', {
      callId,
      channelName: callData.channelName,
      userId: callData.userId,
      providerId: callData.providerId
    ...
    if (io) {
      ...
      // METHOD 1: Emit to specific channel (if user joined the channel)
      const channelSockets = channelToSockets?.get(callData.channelName);
      if (channelSockets && channelSockets.size > 0) {
        io.to(callData.channelName).emit('call-declined', declinePayload);
        console.log(`✅ Decline sent to channel ${callData.channelName} (${channelSockets.size} sockets)`);
      }

      // METHOD 2: Emit to specific user socket (if we have userId)
      if (callData.userId && connectedUsers) {
        const userSocketId = connectedUsers.get(callData.userId);
        if (userSocketId) {
          io.to(userSocketId).emit('call-declined', declinePayload);
          console.log(`✅ Decline sent to user socket ${userSocketId}`);
        } else {
          console.warn(`⚠️ User ${callData.userId} not found in connected users`);
        }
      }

      // METHOD 3: Broadcast globally as ultimate fallback
      io.emit('call-declined', declinePayload);
      console.log(`📢 Global decline broadcast sent for call ${callId}`);
    } else {
      console.error('❌ Socket.IO instance not available');
    }

    // Remove from active calls after brief delay (allow time for notification delivery)
    setTimeout(() => storage.deleteCall(callId), 2000);
  ...
  try {
    ...
    if (callData) {
      ...
      if (io) {
        ...
        // Emit to channel
        io.to(callData.channelName).emit('call-ended', endPayload);
        
        // Also broadcast globally
        io.emit('call-ended', endPayload);
      }
