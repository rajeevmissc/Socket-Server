...
router.post('/sessions', authenticateToken, async (req, res) => {
  try {
    ...
    // ✅ FIXED: Get io instance and emit notification
    const io = req.app.get('io');
    const connectedProviders = req.app.get('connectedProviders');
    const providerSocketId = connectedProviders.get(providerId);

    console.log(`📝 Created chat session ${session._id}`);
    console.log(`🔍 Looking for provider ${providerId}`);
    console.log(`Provider socket ID: ${providerSocketId}`);
    console.log(`All connected providers:`, Array.from(connectedProviders.keys()));
    ...
    if (providerSocketId) {
      ...
      console.log(`⚠️ Provider ${providerId} is not connected - notification skipped`);
    }
...
router.post('/sessions/:sessionId/join', authenticateToken, async (req, res) => {
  try {
    ...
    console.log(`✅ Provider ${userData.providerId} joined session ${session._id}`);
...
router.post('/messages', authenticateToken, async (req, res) => {
  try {
    ...
    // ✅ FIXED: Notify provider if message is from user
    if (userData.role === 'user') {
      const connectedProviders = req.app.get('connectedProviders');
      const providerSocketId = connectedProviders.get(session.providerId);
      ...
      if (providerSocketId) {
        io.to(providerSocketId).emit('newMessageNotification', {
          ...
          roomId: session.roomId,
          senderName: userData.name || 'User',
          messageText: message.length > 50 ? message.substring(0, 50) + '...' : message,
          userId: userData.id,
          timestamp: new Date()
        });
        console.log(`📩 Notified provider ${session.providerId} about new message`);
      }
    }

    // ✅ FIXED: Notify user if message is from provider
    if (userData.role === 'provider') {
      ...
      if (userSocketId) {
        ...
        console.log(`📩 Notified user ${session.userId} about new message`);
      }
    }
