...
// Make io accessible in routes
app.set('io', io);
app.set('connectedProviders', io.connectedProviders);
app.set('connectedUsers', io.connectedUsers);
...
app.get('/api/health', (req, res) => {
  res.json({
    ...
    version: process.env.npm_package_version || '1.0.0',
    database: mongoose.connection.readyState === 1 ? 'connected' : 'disconnected',
    connectedProviders: io.connectedProviders.size,
    connectedUsers: io.connectedUsers.size
  });
});
...
connectToDatabase().then(() => {
  httpServer.listen(PORT, () => {
    ...
    console.log(`Frontend URL: ${process.env.FRONTEND_URL || 'http://localhost:3000'}`);
    console.log(`Socket.IO enabled on port ${PORT}`);
    console.log(`✅ Chat feature integrated successfully`);
    console.log(`📊 Connected providers: ${io.connectedProviders.size}`);
    console.log(`👥 Connected users: ${io.connectedUsers.size}`);
  });
}).catch((err) => {
  console.error("❌ Failed to connect to MongoDB:", err);
  process.exit(1);
