...
providerSchema.index({ 'availability.status': 1 });
providerSchema.index({ 'presence.availabilityStatus': 1 });
// Add these indexes to your provider schema
providerSchema.index({ 'services.category': 1, 'address.city': 1 });
providerSchema.index({ 'ratings.overall': -1, 'availability.status': 1 });
providerSchema.index({ 'address.city': 1, 'services.primary': 1 });
providerSchema.index({ 'professional.experience': -1, 'ratings.overall': -1 });
providerSchema.index({ 'businessInfo.lastActive': -1 });
// Pre-save middleware to set fullName and other auto-generated fields
providerSchema.pre('save', function(next) {
  if (this.personalInfo.firstName && this.personalInfo.lastName) {
    this.personalInfo.fullName = `${this.personalInfo.firstName} ${this.personalInfo.lastName}`;
